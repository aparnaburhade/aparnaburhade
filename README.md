<h1 align="left">
  Hi, I'm Aparna <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="35">
</h1>

<p align="left">
  <img src="https://readme-typing-svg.herokuapp.com/?lines=Software+Engineer;Full-Stack+Developer;AI+Enthusiast;Automation+Builder&left=true&width=500&height=45">
</p>

Software Engineer focused on building scalable backend systems, automation workflows, and AI-driven applications.

I enjoy building systems that combine **APIs, automation, data, and AI** to solve real-world problems.

Currently working on backend infrastructure, automation pipelines, and conversational AI workflows.

---

## 🚀 About Me

- 🎓 Master's in Computer Engineering — University of Central Florida
- 💻 Backend & Full-Stack Developer
- 🤖 Interested in AI systems, automation, and scalable backend architecture
- 🔧 Building APIs, automation tools, and AI-powered applications

---

## 🛠 Tech Stack

**Languages**

Python • JavaScript • C++ • SQL

**Backend**

Node.js • Express • FastAPI • REST APIs

**Frontend**

React • HTML • CSS

**Databases**

PostgreSQL • Redis

**Cloud & Tools**

AWS • Docker • GitHub Actions • Linux

**AI / Data**

LangChain • RAG Pipelines • Embeddings • Vector Search

---

## 📌 Featured Projects

### 🤖 AI Financial Copilot
AI assistant that analyzes financial data and generates insights using agentic workflows and data tools.

### 🔎 EdgeProbe
EdgeProbe is an AI evaluation tool that analyzes LLM-generated responses at a claim level to detect possible hallucinations.
---

## 📊 GitHub Stats

![Aparna's GitHub stats](https://github-readme-stats-git-masterrstaa-rickstaa.vercel.app/api?username=aparnaburhade&show_icons=true&theme=tokyonight)

![Top Languages](https://github-readme-stats-git-masterrstaa-rickstaa.vercel.app/api/top-langs/?username=aparnaburhade&layout=compact&theme=tokyonight)


---

## 🌐 Connect With Me

LinkedIn: https://www.linkedin.com/in/aparna-mahendrakumar-burhade-0819841b2/
Email: aparnaburhade@gmail.com

---

